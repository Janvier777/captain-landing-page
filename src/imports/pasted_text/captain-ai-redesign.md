...
Small pill badge at top: "Now onboarding design partners" (keep this, it's nice)
"Captain." in 80px bold indigo — the biggest text on the entire page
"your team's 24/7 command center." in 28px, light weight (300 or 400), muted lavender-gray (#7B6F99) — NOT bold, much lighter than current
One line of description in 17px warm gray, max-width 600px, centered
The task input mockup card (glassmorphism) with "Make a pitch deck for our Series A." — this should be prominent, roughly 700px wide
"Get Early Access" button + "Watch it work ↓" link
"Built for teams of 3–15" in small muted text
...
Add subtle section dividers: a thin horizontal line (1px, 8% opacity white) or a slight background gradient shift between sections. Currently sections bleed into each other — the reader can't tell where one ends and another begins.
Add a floating "scroll indicator" in the hero: a small animated down-arrow or chevron near the bottom of the hero section, pulsing gently, to invite scrolling.
Consider adding a sticky progress indicator: a thin horizontal bar at the very top of the page (above the nav) that fills as the user scrolls, showing how far down the page they are. Color: a subtle lavender-to-peach gradient, 3px tall.
The nav bar should have slightly more internal padding (16px vertical → 20px) and the nav links should be 15px, not 14px.
Add anchor link highlighting: when the user scrolls to a section, the corresponding nav link should subtly change color (this will be done in code, but design the "active" state — perhaps the link turns indigo with a subtle underline).
